...
class VDSR(nn.Module):
    def __init__(
        self, num_channels=3, num_filters=64, scaling_factor=0.1, num_convs=18
    ):
        super(VDSR, self).__init__()
        self.conv_in = nn.Conv2d(num_channels, num_filters, kernel_size=3, padding=1)
        self.convs = nn.ModuleList(
            [self._build_conv(num_filters) for _ in range(num_convs)]
        )
...
class EDSR(nn.Module):
    def __init__(
        self, num_channels=3, num_filters=64, scaling_factor=0.1, num_residuals=20
    ):
